...
class TheGym:
    ...
    def get_angle(self, a, b):
        ...
        Args:
            a (numpy array): Coordinates of point A as a 2D numpy array.
            b (numpy array): Coordinates of point B as a 2D numpy array.

        Returns:
            int: 180 if A is to the left of B, 100 if A is to the right of B.
        """
        a = np.array(a)
        b = np.array(b)
        ...
        # If A is to the left of B, return 180
        if b[0] - a[0] > 0:
            return 180
        # If A is to the right of B, return 100
        else:
            return 100
    ...
    def start_counting(self, im0, results, frame_count):
        ...
        for ind, k in enumerate(reversed(self.keypoints)):
            ...
            if self.pose_type == "walk":
                self.angle[ind] = self.get_angle(k[int(self.kpts_to_check[0])].cpu(),
                                                 k[int(self.kpts_to_check[1])].cpu())
                self.im0 = self.annotator.draw_specific_points(k, self.kpts_to_check, shape=(640, 640), radius=10)
            ...
            if self.pose_type == "walk":
                if self.angle[ind] == 180:
                    if self.stage[ind] == "right":
                        self.count[ind] += 1
                    self.stage[ind] = "left"

                if self.angle[ind] == 100 and self.stage[ind] == "left":
                    self.stage[ind] = "right"
                    self.count[ind] += 1
                self.annotator.plot_angle_and_count_and_stage(
                    angle_text=self.angle[ind],
                    count_text=self.count[ind],
