...
class DelMessageBox(MessageBoxBase):
    ...
    def __init__(self, parent=None):
        ...
        self.titleLabel = SubtitleLabel('删除帖子', self)
        self.urlLineEdit = LineEdit()

        self.urlLineEdit.setPlaceholderText('输入要删除帖子的uid')
    ...
    def __onYesButtonClicked(self):
        ...
        if not um.query_post_by_post_id(self.urlLineEdit.text()):
            InfoBar.warning(
                title='Error',
                content="Invalid postid.",
            ...
            return 0
        um.delete_post(self.urlLineEdit.text())

        self.accept()
        self.accepted.emit()


class PostTableCard(SimpleCardWidget):
    def __init__(self, parent=None):
        super().__init__(parent)

        self.userTable = TableWidget(self)

        self.delBtn = PrimaryPushButton('Delete Post', self)
    ...
    def initPostTable(self):
        um = PostManageDatabaseFactory()
        results = um.query_all_posts()
        ...
        # 设置表头标签
        self.userTable.setHorizontalHeaderLabels(['PostID', 'UserID', 'Title', 'Content'])
...
class ManagePostWidget(QWidget):
    def __init__(self, parent=None, userid=None):
        super().__init__(parent)
        self.setObjectName("ManagePostWidget")
        self.userid = userid
    ...
    def initUI(self):
        ...
        self.postTableCard = PostTableCard(self)

        self.Hlayout.addStretch(2)
        self.Hlayout.addWidget(self.postTableCard, 6)
