...
class VideoPlayer(QMainWindow):
    ...
    def open_camera(self):
        ...
        self.gym_object = TheGym()
        self.gym_object.set_args(line_thickness=2, view_img=False, pose_type="walk", kpts_to_check=[15, 16, 11],
                                 pose_down_angle=117)
        self.frame_count = 0
        self.is_paused = False
        self.start_time = time.time()
    ...
    def update_frame(self):
        if not self.is_paused and self.cap is not None and self.cap.isOpened():
            ...
            self.plan_card.BurnedCLabel.setText(
                "Calories Burned: {:.1f}".format(self.gym_object.count[0] * 0.17) + " kcal")
        else:
            self.pause_video()
    ...
    def closeEvent(self, event):
        ...
        if self.elapsed_time != 0:
            self.parent().show_and_add_record(self.parent().parent().userid, 2, self.gym_object.count[0], self.gym_object.count[0] * 0.17, self.elapsed_time)
        pm = PlanManageDatabaseFactory()
        pm.complete_task(self.parent().parent().userid, 2)
